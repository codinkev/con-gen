...
//http://stackoverflow.com/questions/4572338/extending-application-to-share-variables-globally
	...
	DatabaseHelper sqldb;
	   ...
	   //startup method: if tables have already been created, don't recreate unless asked to update explicitly
	   //for now avoid this -- have them only explicitly refresh even if not exist
	   public void checkIfExist(){
	   }
	
	   //desired functionality: completely erase and re-update regardless if tables exist
	   public void totallyRefresh() {
	   sqldb = DatabaseHelper.getInstance(getApplicationContext());
	   sqldb.recreateTables();
		
		//we can't have contacts run asynchronously because noncons depends on it
		getContacts();
		
		//now that we can check against existing cons, run text and call refresh asynchronously
		//this is the major overhead load anyway (more texts/calls than contacts)
	   ...
	   private void getContacts() {
	 			...
	 			while (cursor.moveToNext()) {
	 		        ...
	 		        if (Integer.parseInt(cursor.getString(
	 		           //iterating over EACH number associated with EACH name (internal cursor)
	 		...
	 		private void getTextDetails() { 			
	 			...
	 			/* FIELDS: (this is an undocumented content provider)
	 			0: _id
	 			1: thread_id
	 			2: address
	 			3: person
