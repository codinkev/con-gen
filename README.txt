...
***OVERVIEW OF CLASSES - 
  ...
  in Activities section
    ...
    NonContactList - listview displaying all numbers which are not contacts yet but have texts/calls on phone (all noncons).  
    Clicking number on this listview for a number brings us to NonConExplorer
      
    NonConExplorer - for a given number clicked on in NonContactList, can see all text/call activity and also have ability to 
    add the contact if you recognize info and want to add them; can also totally delete the number's data from the phone
    
    AddConFrag - independent component for new contact data entry and insertion (to main contacts content provider on phone)
      
    ContactList - activity which displays and allow you to access existing contacts by clicking on number in listview.  
    
    ContactExplorer - for a given number clicked on in ContactList, can see all text/call activity and also totally delete the contact from phone
  ...
  in Utilties section
    ...
    UpdateService - contains methods for accessing content providers and storing in database; enables us to run
    database refreshes in the background
    
    UpdateStatusBroadcast - receiver for telling current activity that the updateservice has completed
